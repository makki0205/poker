  ...
  return (
    <main className="page">
      ...
      <section className="card">
        ...
        <div className="cards">
          {snapshot?.board.map((card, idx) => (
            <span className="chip" key={`${idx}-${card.rank}-${card.suit}`}>
              {renderCard(card)}
            </span>
          ))}
        </div>
      ...
      <section className="card">
        ...
        <div className="seatList">
          {snapshot?.seats.map((seat) => (
            <article className="seat" key={seat.playerId}>
              <header>
                <strong>
              ...
              <div className="cards">
                {seat.holeCards?.map((card, idx) => (
                  <span className="chip" key={`${seat.playerId}-${idx}-${card.rank}-${card.suit}`}>
                    {renderCard(card)}
                  </span>
      ...
      {props.role === 'player' ? (
        <section className="card">
          <h2>Actions</h2>
          <p className="muted">{isMyTurn ? 'Your turn' : 'Waiting for your turn'}</p>
          ...
          {me && me.stack <= 0 ? (
            <button onClick={props.onRebuy}>Rebuy</button>
          ) : null}
        </section>
      ) : (
        <section className="card">
